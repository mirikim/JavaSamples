...
public class EventTest {
	static int i = 0;// 이벤트가 발생했을 때 호출되는 메서드의 번호에사용.
	...
	public static void main(String[] args) {
		...
		/// Frame에 메뉴를 추가한다.
		// Menu와 MenuItem은 Event23클래스를 사용하지 않고, 익명으로처리
		...
		addAllListener(f);// Frame에 Listener를 추가한다.
		f.setVisible(true);
	...
	static void addAllListener(Component c) {
		...
		// Event가 발생하면, 호출된 메서드의 이름과 EventSource에 대한 정보를 출력하도록했다.

		// 내부클래스 ->지역클래스// 메소드내에서 지역변수처럼 사용되는
		class EventHandler implements ActionListener, AdjustmentListener, ComponentListener, ContainerListener,
				FocusListener, ItemListener, KeyListener, MouseListener, MouseWheelListener, MouseMotionListener,
			...
			public void mouserDragged(MouseEvent e) {
			...
			// WindowFocusListener의 메서드(JDk1.4부터 추가)
			...
			// WindowStateListener의 메서드(JDK 1.4부터추가)
